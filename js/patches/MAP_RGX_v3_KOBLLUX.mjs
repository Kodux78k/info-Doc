...
/* === BUTTON ENGINE === */
// Aceita IDs com ponto: 2.2.loop, 2.2.pulse etc.
function ButtonEngine(root=document){
  const RX_BTN = /\[\[btn:(?<act>[a-z0-9._-]+)(?:\|(?<label>[^\]]+))?\]\]/gi;
  root.querySelectorAll('p,li,blockquote,td,th,h1,h2,h3,h4,h5,h6,div').forEach(el=>{
    ...
    el.innerHTML = html.replace(RX_BTN, (m)=>{
      const g = m.match(/\[\[btn:([a-z0-9._-]+)(?:\|([^\]]+))?\]\]/i);
      const act = (g&&g[1])||'act';
...
/* === IA BRIDGE EXTENDED (gerar, loop, tts, pulse) === */
// Aqui o ID "2.2.loop" vira "loop" antes do map.
ensureBus().on('btn', async ({id,rawId,ctx})=>{
  try{
    ...
    const baseId = id || (rawId ? String(rawId).split('.').pop() : '');
    const map = { gerar:'complete', loop:'continue', tts:'speak', pulse:'speak' };
    let mode  = map[baseId] || 'complete';

    if(baseId === 'gerar' && ctx.text){
...
document.addEventListener('click', e=>{
  ...
  const rawId = b.dataset.btn || '';
  const baseId = rawId.split('.').pop(); // 2.2.loop -> loop
  ...
  ensureBus().emit('btn',{id: baseId, rawId, ctx});
}, true);
